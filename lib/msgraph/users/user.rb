...
class Msgraph::Users::User < Msgraph::Users::Base

  def initialize(args = {})
    raise Msgraph::Users::UserError.new("Does not exist ':token' in arguments.") unless args.key?(:token)
    @token = args[:token]
    ...
    if @select.size > 0
      query.merge!({ '$select' => @select.map { |key_name|
                       if key_name.is_a?(Symbol)
                         ...
                         raise Msgraph::Users::UserError.new("'#{key_name}' is invalid value.")
                       end
                     }.join(',') })
    end

    client = HTTPClient.new
    response = client.get("#{@base_url}", query, header)
    case response.code
    when 200
  ...
  def get(args = {})
    raise Msgraph::Users::UserError.new("Does not exist ':id' or ':user_principal_name'.") unless args.key?(:id) || args.key?(:user_principal_name)
    id = args[:id] || args[:user_principal_name]
    client = HTTPClient.new
    query = {}
    response = client.get("#{@base_url}/#{id}", query, header)
    case response.code
    when 200
      body = JSON.parse(response.body)
    when 202
...
class Msgraph::Users::UserError < StandardError; end
